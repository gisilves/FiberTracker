...
void fiberCluster(int runnumber, int board, int highThresh, int lowThresh)
{
  ...
  TH1F *h_adc = new TH1F("h_adc", "h_adc", 1000, -0.5, 2000.5);
  ...
  myfile << "Pedestals    Raw Sigmas\n";

  for (int i = 0; i < pedestals.size(); i++)
  {
    myfile << pedestals.at(i) << " ; " << raw_sigmas.at(i) << "\n"; //write to file
  }
  myfile.close();
  ...
  // c2->cd(3);
  // frame = gPad->DrawFrame(0, 0,32,10);
  // gPad->SetGridy(1);
  // frame->SetTitle("Event; channel; (adc)");
  // frame->GetYaxis()->SetTitleOffset(1.55);
  // frame->GetXaxis()->SetNdivisions(4,5,1,kFALSE);
  // //gr_event->SetGrid();
  // gr_event->Draw("B");
